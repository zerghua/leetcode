 ...
 * Created by Hua on 5/24/2017.
 ...
 the target value for each machine is 4. Convert the machines array to a kind of gain/lose array,
 ...
 and the max of "gain/lose" array. In this case, the peak of abs(cnt) is 8 and the max of gain/lose array is 7.
 So the result is 8.
...
public class N517_SuperWashingMachines {
    ...
    public class Solution {
        public int findMinMoves(int[] machines) {
            ...
            int avg = total / machines.length, count = 0;
            for(int i: machines){
                count += i - avg;
                ret = Math.max(ret, Math.max(i - avg, Math.abs(count)));
            }
            return ret;
        }
    }

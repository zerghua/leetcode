 ...
 * Created by Hua on 6/27/2017.
 ...
 You can rotate the ring clockwise or anticlockwise one place, which counts as 1 step. The final purpose of
 the rotation is to align one of the string ring's characters at the 12:00 direction, where this character
 must equal to the character key[i].
...
public class N514_FreedomTrail {
    ...
    public class Solution2 {
        HashMap<String, HashMap<Integer, Integer>> memo;

        public int findRotateSteps(String ring, String key) {
            memo = new HashMap();
            return key.length() + dfs(ring, key, 0);
        }


        public int dfs(String ring, String key, int start){
            if(start == key.length()) return 0;
            if(memo.containsKey(ring) && memo.get(ring).containsKey(start)) return memo.get(ring).get(start);
            int f = ring.indexOf(key.charAt(start));
            int b = findPosBackward(ring, key.charAt(start));
            int clock = f + dfs(ring.substring(f) + ring.substring(0,f), key, start+1);
            int anticlock = ring.length()-b + dfs(ring.substring(b) + ring.substring(0,b), key, start+1);
            int ret = Math.min(clock, anticlock);
            if(!memo.containsKey(ring)) memo.put(ring, new HashMap());
            memo.get(ring).put(start, ret);
            return ret;
        }

        public int findPosBackward(String ring, char c){
            if(ring.charAt(0) == c) return 0;
            for(int i=ring.length()-1; i>0; i--){
                if(ring.charAt(i) == c) return i;
            }
            return 0;
        }
    }

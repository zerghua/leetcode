 ...
 * Created by Hua on 6/21/2017.
 ...
 it is valid. A code snippet is valid if all the following rules hold:


 The code must be wrapped in a valid closed tag. Otherwise, the code is invalid.

 1. A closed tag (not necessarily valid) has exactly the following format : <TAG_NAME>TAG_CONTENT</TAG_NAME>. Among them,
 <TAG_NAME> is the start tag, and </TAG_NAME> is the end tag. The TAG_NAME in start and end tags should be the same.

 2. A closed tag is valid if and only if the TAG_NAME and TAG_CONTENT are valid.
 ...
 Inspired by @StefanPochmann 's idea in this post
 ...
 Use Group (([A-Z]{1,9}) then \\1) when matching TAG_NAME.
